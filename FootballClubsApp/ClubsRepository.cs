...
public class ClubsRepository
{
    ...
    public bool HasPlayers(int clubId)
    {
        string sql = "SELECT PlayerId FROM players WHERE ClubId=@id LIMIT 1";

        DataTable dt = Db.GetDataTable(sql, new MySqlParameter("@id", clubId));
    ...
    public void Delete(int id)
    {
        if (HasPlayers(id))
        {
            throw new Exception("Този отбор има играчи и не може да бъде изтрит.");
        }
